...
export = async () => {
	...
	return all([servicecatalogOutput, resourcegroupsOutput, snsOutput]).apply(
		([servicecatalog, resourcegroups, snsTopics]) => {
			const exported = {
				paloma_application_servicecatalog: servicecatalog,
				paloma_application_resourcegroups: resourcegroups,
				paloma_application_sns: snsTopics,
			} satisfies z.infer<typeof PalomaApplicationStackExportsZod>;
