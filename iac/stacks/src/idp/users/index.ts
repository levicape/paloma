...
import {
	PalomaIdpUsersCognitoDomain,
	PalomaIdpUsersStackExportsZod,
} from "./exports";

const PACKAGE_NAME = "@levicape/paloma";
const COGNITO_ROOT_DOMAIN = PalomaIdpUsersCognitoDomain;
...
export = async () => {
	...
	return all([userpoolsOutput]).apply(([userpools]) => {
		const exported = {
			paloma_idp_users_cognito: {
				operators: userpools.operators,
			},
		} satisfies z.infer<typeof PalomaIdpUsersStackExportsZod>;

		const validate = PalomaIdpUsersStackExportsZod.safeParse(exported);
		if (!validate.success) {
