...
import {
	PalomaNevadaChannelsStackExportsZod,
	PalomaNevadaChannelsStackrefRoot,
...
import {
	PalomaNevadaWWWRootExportsZod,
	PalomaNevadaWWWRootSubdomain,
} from "./exports";
...
const STACKREF_CONFIG = {
	[STACKREF_ROOT]: {
		...
		[PalomaDnsRootStackrefRoot]: {
			refs: {
				route53: PalomaDnsRootStackExportsZod.shape.paloma_dns_root_route53,
				acm: PalomaDnsRootStackExportsZod.shape.paloma_dns_root_acm,
			},
		},
		[PalomaNevadaChannelsStackrefRoot]: {
			refs: {
				sns: PalomaNevadaChannelsStackExportsZod.shape
					.paloma_nevada_channels_sns,
			},
		},
		[PalomaNevadaHttpStackrefRoot]: {
			refs: {
...
function handler(event) {
  ...
  return event.request;
	...
	// TLS
	//////
	const acm = dereferenced$[PalomaDnsRootStackrefRoot]?.acm;
	const certificate =
	...
	(() => {
		const revalidateTopicArn =
			dereferenced$[PalomaNevadaChannelsStackrefRoot].sns.revalidate.topic.arn;
	...
	// DNS Record
	const route53 = dereferenced$[PalomaDnsRootStackrefRoot].route53;
	...
	const $nevada_http = dereferenced$[PalomaNevadaHttpStackrefRoot];
	const $nevada_web = dereferenced$[PalomaNevadaWebStackrefRoot];

	return all([s3Output, cloudfrontOutput, codebuildProjectsOutput]).apply(
		([
			paloma_nevada_wwwroot_s3,
			paloma_nevada_wwwroot_cloudfront,
			paloma_nevada_wwwroot_codebuild,
		]) => {
			const exported = {
				paloma_nevada_wwwroot_imports: {
					paloma: {
						[PalomaNevadaHttpStackrefRoot]: $nevada_http,
						[PalomaNevadaWebStackrefRoot]: $nevada_web,
					},
				},
				paloma_nevada_wwwroot_s3,
				paloma_nevada_wwwroot_cloudfront,
				paloma_nevada_wwwroot_codebuild,
			} satisfies z.infer<typeof PalomaNevadaWWWRootExportsZod> & {
				paloma_nevada_wwwroot_imports: {
					paloma: {
						[PalomaNevadaHttpStackrefRoot]: typeof $nevada_http;
						[PalomaNevadaWebStackrefRoot]: typeof $nevada_web;
					};
				};
			};

			const validate = PalomaNevadaWWWRootExportsZod.safeParse(exported);
			if (!validate.success) {
