...
const DESCRIPTION = "Provides Paloma execution data to Nevada UI" as const;
	...
	[STACKREF_ROOT]: {
		...
		codestar: {
			refs: {
				...
				codeartifact:
					PalomaCodestarStackExportsZod.shape.paloma_codestar_codeartifact,
				ssm: PalomaCodestarStackExportsZod.shape.paloma_codestar_ssm,
			},
		},
		datalayer: {
			refs: {
				props: PalomaDatalayerStackExportsZod.shape.paloma_datalayer_props,
				ec2: PalomaDatalayerStackExportsZod.shape.paloma_datalayer_ec2,
				efs: PalomaDatalayerStackExportsZod.shape.paloma_datalayer_efs,
...
const ENVIRONMENT = (
	...
	const idpUsers = (() => {
		const { cognito } = _$refs[PalomaIdpUsersStackrefRoot];
		return {
	...
	return {
		PALOMA_DATALAYER_MOUNT_PATH: efs.mountPath,
...
export = async () => {
	...
	return all([
		...
		([
			paloma_nevada_http_s3,
			paloma_nevada_http_cloudwatch,
			paloma_nevada_http_lambda,
			paloma_nevada_http_cloudmap,
			paloma_nevada_http_codebuild,
			paloma_nevada_http_codepipeline,
			paloma_nevada_http_eventbridge,
		]) => {
				...
				> = {
					["/~/Paloma/Nevada"]: {
						...
						lambda: {
							...
							name: paloma_nevada_http_lambda.function.name,
							role: {
								arn: paloma_nevada_http_lambda.role.arn,
								name: paloma_nevada_http_lambda.role.name,
							},
						...
						cloudmap: {
							namespace: {
								arn: paloma_nevada_http_cloudmap.namespace.arn,
								name: paloma_nevada_http_cloudmap.namespace.name,
								id: paloma_nevada_http_cloudmap.namespace.id,
								hostedZone: paloma_nevada_http_cloudmap.namespace.hostedZone,
							},
							service: {
								arn: paloma_nevada_http_cloudmap.service.arn,
								name: paloma_nevada_http_cloudmap.service.name,
							},
							instance: {
								id: paloma_nevada_http_cloudmap.instance.id,
								attributes: paloma_nevada_http_cloudmap.instance.attributes,
							},
						},
					},
				};
			...
			const exported = {
				...
				paloma_nevada_http_codepipeline,
				paloma_nevada_http_eventbridge,
				paloma_nevada_http_routemap,
			} satisfies z.infer<typeof PalomaNevadaHttpStackExportsZod> & {
				paloma_nevada_http_imports: {
					[PalomaApplicationRoot]: {
