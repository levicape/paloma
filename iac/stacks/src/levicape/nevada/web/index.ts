...
import {
	...
	PalomaNevadaClientStackrefRoot,
} from "../client/exports";
	...
	[STACKREF_ROOT]: {
		application: {
			refs: {
				...
				sns: PalomaApplicationStackExportsZod.shape.paloma_application_sns,
			},
		},
		codestar: {
			refs: {
		...
		[PalomaNevadaClientStackrefRoot]: {
			refs: {
				cognito:
					PalomaNevadaClientStackExportsZod.shape.paloma_nevada_client_cognito,
			},
		},
		[PalomaNevadaChannelsStackrefRoot]: {
			refs: {
...
export = async () => {
	...
	const extractimage = (() => {
		...
		const { domain } =
			dereferenced$[PalomaIdpUsersStackrefRoot].cognito.operators; // *.idp.az.
		const { client: oidcClient } =
			dereferenced$[PalomaNevadaClientStackrefRoot].cognito.operators;
								...
								...(domainName !== undefined
									? [
											...objectEntries({
												...
												OAUTH_PUBLIC_OIDC_POST_LOGOUT_REDIRECT_URI: `https://${domainName}/${PalomaNevadaClientOauthRoutes.logout}`,
												OAUTH_PUBLIC_OIDC_SILENT_REDIRECT_URI: `https://${domainName}/${PalomaNevadaClientOauthRoutes.renew}`,
											}).flatMap(([key, value]) => [
												`export ${key}="${value}"`,
												`echo $${key}`,
											]),
										...
										return {
											...
											EnvironmentVariables: JSON.stringify([
												...
												},
												{
													name: "SNS_REVALIDATE_TOPIC",
													value:
														dereferenced$[PalomaNevadaChannelsStackrefRoot].sns
