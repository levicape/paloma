...
import { PalomaDnsRootStackExportsZod } from "./exports";
...
export = async () => {
	...
	if (!domainName) {
		...
		return {
			paloma_dns_root_route53: {},
			paloma_dns_root_acm: {},
		} satisfies z.infer<typeof PalomaDnsRootStackExportsZod>;
	}
	...
	return all([route53Output, acmOutput]).apply(([route53, acm]) => {
		const exported = {
			paloma_dns_root_route53: route53,
			paloma_dns_root_acm: acm,
		} satisfies z.infer<typeof PalomaDnsRootStackExportsZod>;

		const validate = PalomaDnsRootStackExportsZod.safeParse(exported);
		if (!validate.success) {
